...
if getgenv()._carfly_erx then return end
getgenv()._carfly_erx = true
...
getgenv()._carfly_api = {
	toggle = function(state)
		...
		on = state
		-- Toggle BorderWalls collision with CarFly state
		pcall(function()
			local BorderWalls = game:GetService("Workspace"):FindFirstChild("BorderWalls")
			if BorderWalls then
				for _, part in ipairs(BorderWalls:GetDescendants()) do
...
local WindUI = _G.WindUI
if WindUI then WindUI:Notify({ Title = "CarFly (Keyboard) Loaded", Content = "CarFly is ready!", Duration = 3, Icon = "plane" }) end
